...
onLoad((options) => {
	...
	getVideo({
		...
		if (!(res.nextId == 0 && cateIndex == tabList.length)) {
			getVideo({
				id: res.nextId,
			}).then((res) => {
...
function f_change(e) {
	...
	if (dir.value === "next") {
		...
		if (id == 0 && cateIndex < tabList.length) {
			...
			preIdList.push(videoInfoList.value[(current.value + 1) % 5].id);
		}
		if (!(id == 0 && cateIndex == tabList.length)) {
			getVideo({
				id,
				...
				console.log(target, "下");
				target.id = res.xcxVideo.id;
				target.nextId = res.nextId;
				target.prevId = res.prevId;
				if (res.xcxVideo.video) {
					target.src = res.xcxVideo.video;
				} else {
					target.src = await returnData(res.xcxVideo.videoUrl);
				}

				console.log(videoInfoList.value, "xia");
			});
		}
	} else {
		console.log("上", current.value, current.value < 1 ? 4 : current.value - 1);
		let id = videoInfoList.value[current.value < 1 ? 4 : current.value - 1].prevId;
		if (id == 0 && cateIndex > 0) {
			cateId.value = tabList[cateIndex - 1]?.baseVideoType;
			id = preIdList[cateIndex - 1];
		}
		if (id != 0) {
			getVideo({
				id,
...
function returnData(vid: string) {
	return new Promise((resolve) => {
		uni.request({
...
function loadVideo() {}
function playVideo() {}
function onVideoTimeUpdate(e, index: number) {
	const time = e.detail.currentTime;
	if (time > 0) {
		videoRefList.value[index].pause();
